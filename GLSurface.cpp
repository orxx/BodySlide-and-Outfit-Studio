...
void GLSurface::initMaterial(const vec3& diffusecolor) {
	float spec[] = { 0.4f, 0.4f, 0.4f, 1.0f };
	...
	if (meshes.size() > 0 && activeMesh >= 0) {
		...
		if (meshes[activeMesh]->bvh->IntersectRay(o, d, &results)) {
			...
			if (results.size() > 0) {
				...
				/*
				if(meshes[activeMesh]->vertTris) {
					AddVisFacets(meshes[activeMesh]->vertTris[pointid],"adjacency");
					if(meshes[activeMesh]->weldVerts.find(pointid) !=meshes[activeMesh]->weldVerts.end() ){						
						AddVisFacets(meshes[activeMesh]->vertTris[meshes[activeMesh]->weldVerts[pointid][0]],"adjacencyb");
					} else {
						DeleteMesh("adjacencyb");
					}

				}
				*/
				//AddVisTri(p1,p2,p3,"trihilite");
			}
		} else {
			ShowCursor(false);
		}
			...
			GetPickRay(vpW/2, vpH/2, v, vo);
			v *= -1;
			//float d = v.dot(vec3(0,0,0));
			if(CollidePlane(ScreenX,ScreenY,o,v,0)) {
				AddVisCircle(o,v,0.5f,"cursormesh");
			}
		}// */
	}
	return ret;
...
/*	if (0) {	// bUseVBOs     too many problems: AMD drivers required double buffer loads and ran into some artifacting. Vertex arrays seem just as speedy for this.
		if (!m->bBuffersLoaded) {
			glBindBuffer(GL_ARRAY_BUFFER,m->VboBufName);
			glBufferData( GL_ARRAY_BUFFER, sizeof(vtx)*m->nVerts, &m->verts[0].x, GL_STREAM_DRAW );
			glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(vtx)*m->nVerts,&m->verts[0].x);
				
			glBindBuffer(GL_ELEMENT_ARRAY_BUFFER,m->IboBufName);
			glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(tri)*m->nTris,&m->tris[0].p1,GL_STREAM_DRAW );
			glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(vtx)*m->nTris,&m->tris[0].p1);
			m->bBuffersLoaded = true;
		}	
		glBindBuffer(GL_ARRAY_BUFFER, m->VboBufName );

		glEnableClientState(GL_VERTEX_ARRAY);
		glVertexPointer(3,GL_FLOAT,sizeof(vtx),0);
			
		glEnableClientState(GL_NORMAL_ARRAY);
		glNormalPointer(GL_FLOAT,sizeof(vtx),(void*)12);

		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER,m->IboBufName);
		glDrawElements(GL_TRIANGLES,(m->nTris)*3,GL_UNSIGNED_SHORT,0);
		elemPtr=0;
	} else {
		glEnableClientState(GL_VERTEX_ARRAY);
...
//	m->color = vec3(c,c,c);
	...
	if (0) {  //bUseVBO
		m->VboBufName = 0;
		glGenBuffers(1, &m->VboBufName);

		m->IboBufName = 0;
		glGenBuffers(1, &m->IboBufName);
	}
...
void GLSurface::AddMeshFromNif(NifFile* nif, string shapeName, vec3* color, bool smoothNormalSeams) {
	...
	nif->GetVertsForShape(shapeName, nif_verts);
	//const vector<vector3>* nif_verts = nif->GetVertsForShape(shapeName);
	const vector<triangle>* nif_tris = nif->GetTrisForShape(shapeName);
	...
	const vector<vector3>* nif_norms = 0; //nif->GetNormalsForShape(shapeName);
	const vector<vector2>* nif_uvs = nif->GetUvsForShape(shapeName);
	...
	if (nif_uvs) {
		...
		for (i = 0; i < m->nVerts; i++) {
			m->texcoord[i].u = (*nif_uvs)[i].u;
			m->texcoord[i].v = (*nif_uvs)[i].v;
		}
		m->textured = true;
		// What was this code trying to set the material to?
#if 0
	...
	if (!nif_norms) {
		...
		for (i = 0 ; i < m->nVerts; i++) {
			...
			m->verts[i].nz = (*nif_norms)[i].y;			
			m->verts[i].ny = (*nif_norms)[i].z;
		}
	}
	...
	if (!color) {
		float c;
		for (i = 0; i < meshes.size(); i++) {
			c = 0.4f + ((0.6f / (meshes.size())) * i);
			meshes[i]->color = vec3(c, c, c);
			//meshes[i]->color = vec3(1.0f,1.0f,1.0f);
		}	
	} else {
